...
def findmean(totalweight, totale):
    mean = totalweight / totale
    print(f"The mean is {mean:2f}")

def findmedian(totale, sorteddata):
    if totale % 2 == 0:
        median1 = float(sorteddata[totale//2])
        median2 = float(sorteddata[totale//2 - 1])
        median = (median1 + median2) / 2
    else:
        median = float(sorteddata[totale//2])
...
findmean(totalweight, totale)
findmedian(totale, sorteddata)
findmode(sorteddata)